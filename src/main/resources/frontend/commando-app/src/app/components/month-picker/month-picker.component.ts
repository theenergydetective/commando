...
import {MonthYear, MonthYearRange} from "../../models/month-year";

class MonthPickerData {
  public year:number;
  public months:Array<MonthYear> = [];
}
...
//this.selected.emit(value);
  ...
  ngAfterContentInit(): void {
    ...
    for (let y=this.startYear; y <= thisYear; y++){
      ...
      for (let m=0; m< 12; m++) {
        let my:MonthYear = new MonthYear();
        my.month = m;
        my.year = y;
        my.selected = false;
    ...
    this.onSelected(new MonthYear(new Date(),true));

  }


  onSelected($event: MonthYear) {
    if (this.range.start == null || this.range.end != null){
      ...
      for (let v=0; v < this.values.length; v++){
        let months:Array<MonthYear> = this.values[v].months;
        for (let m=0; m < months.length; m++){
          let my:MonthYear = months[m];
